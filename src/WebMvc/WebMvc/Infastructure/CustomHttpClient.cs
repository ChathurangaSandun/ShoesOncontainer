namespace ShoesOnContainers.WebMvc.Infastructure
{
    ...
    public class CustomHttpClient: IHttpClient
    {
        ...
        {
            return await this.DoPostPutAsync(HttpMethod.Post, uri, item);
        }

        public async Task<HttpResponseMessage> PutAsync<T>(string uri, T item)
        {
            return await this.DoPostPutAsync(HttpMethod.Put, uri, item);
        }

        public async Task<HttpResponseMessage> DeleteAsync(string uri)
        {
            var requestMessage = new HttpRequestMessage(HttpMethod.Delete, uri);
            return await this._client.SendAsync(requestMessage);
        }
    }
}
